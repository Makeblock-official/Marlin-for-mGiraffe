  ...
  inline void gcode_G29() {
    ...
    feedrate = homing_feedrate[Z_AXIS];
      ...
      const int xGridSpacing = (right_probe_bed_position - left_probe_bed_position) / (auto_bed_leveling_grid_points - 1),
        ...
        float z_offset = 0;//zprobe_zoffset;
      ...
      if (!dryrun) {
		...
		current_position[Z_AXIS] = z_tmp - real_z + current_position[Z_AXIS];   //Tshe difference is added to current position and sent to planner.
    ...
    case 'G': switch (codenum) {
      ...
      case 28: // G28: Home all axes, one at a time
        ...
        case 29: // G29 Detailed Z probe, probes the bed at 3 or more points.
          if(code_seen('C'))
		  	reset_bed_level();//G29 C    reset_bed_level
		  else
            gcode_G29();
          break;
      #endif
    ...
    */
    ...
    void adjust_delta(float cartesian[3]) {
     // if (delta_grid_spacing[0] == 0 || delta_grid_spacing[1] == 0) return; // G29 not done!
	  ...
	  {
		delta_grid_spacing[0] = (RIGHT_PROBE_BED_POSITION - LEFT_PROBE_BED_POSITION) / (AUTO_BED_LEVELING_GRID_POINTS - 1);
		delta_grid_spacing[1] = (BACK_PROBE_BED_POSITION - FRONT_PROBE_BED_POSITION) / (AUTO_BED_LEVELING_GRID_POINTS - 1);
	  }
      int half = (AUTO_BED_LEVELING_GRID_POINTS - 1) / 2;
...
//      delta[Y_AXIS] += offset;
//      delta[Z_AXIS] += offset;
	delta[X_AXIS] += offset + zprobe_zoffset;
	delta[Y_AXIS] += offset + zprobe_zoffset;
	delta[Z_AXIS] += offset + zprobe_zoffset;
